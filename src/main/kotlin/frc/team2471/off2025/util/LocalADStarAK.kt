...
class LocalADStarAK : Pathfinder {
    ...
    override fun setDynamicObstacles(
        obs: MutableList<Pair<Translation2d?, Translation2d?>?>?, currentRobotPos: Translation2d?
    ) {
        if (!Logger.hasReplaySource()) {
    ...
    private class ADStarIO : LoggableInputs {
        ...
        override fun fromLog(table: LogTable) {
            ...
            val pointsLogged = table.get("CurrentPathPoints", DoubleArray(0))

            val pathPoints: MutableList<PathPoint> = ArrayList<PathPoint>()
            var i = 0
